...
WITH CTE AS (
  SELECT 
    *,
    ROW_NUMBER() OVER (PARTITION BY stock_name, operation ORDER BY operation_day ASC) AS rn
  FROM Stocks
)

SELECT 
  t1.stock_name,
  SUM(t2.price - t1.price) AS capital_gain_loss
FROM CTE t1
JOIN CTE t2
  ON t1.stock_name = t2.stock_name
  AND t1.rn = t2.rn
  AND t1.operation <> t2.operation
  AND t1.operation_day < t2.operation_day
GROUP BY 1
;
```