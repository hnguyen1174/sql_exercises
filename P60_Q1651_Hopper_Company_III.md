...
```python
all_month = pd.DataFrame(np.linspace(1,12, 12), columns=['month'])
accepted_rides_with_date = pd.merge(
    accepted_rides,
    rides,
    how='left',
    on='ride_id'
)[['ride_id', 'ride_distance', 'ride_duration', 'requested_at']]
accepted_rides_with_date['requested_at'] = pd.to_datetime(accepted_rides_with_date['requested_at'])
accepted_rides_with_date['month'] = accepted_rides_with_date['requested_at'].dt.month
accepted_rides_with_date['year'] = accepted_rides_with_date['requested_at'].dt.year
accepted_rides_with_date = accepted_rides_with_date[accepted_rides_with_date['year'] == 2020]
accepted_rides_with_date_agg = accepted_rides_with_date.groupby('month')[['ride_distance', 'ride_duration']].mean().reset_index()

accepted_rides_with_all_months = pd.merge(
    all_month,
    accepted_rides_with_date_agg,
    on='month',
    how='left'
).fillna(0)

indexer = pd.api.indexers.FixedForwardWindowIndexer(window_size=3)
accepted_rides_with_all_months['avg_ride_distance'] = accepted_rides_with_all_months['ride_distance'].rolling(indexer,  min_periods=3).mean()
accepted_rides_with_all_months['average_ride_duration'] = accepted_rides_with_all_months['ride_duration'].rolling(indexer,  min_periods=3).mean()

results = accepted_rides_with_all_months[['month', 'avg_ride_distance', 'average_ride_duration']].dropna()
print(results)
```